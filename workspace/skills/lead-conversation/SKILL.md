...
description: >-
  ...
  when the meetings link is sent; log LinkedIn reply notes. Do not message unknown leads.
  All replies follow workspace/skills/messaging-guidelines/SKILL.md conversation framework.
  Use when the user wants to handle Sales Navigator conversation replies, inbox follow-up, or
...
Use the **`hubspot`** skill and **`linkedin-sales-navigator`** skill together: open **LinkedIn Sales Navigator** at the **Unread-filtered inbox** URL below, prioritize **unread** threads, **read the prospect's message in full** (and enough thread context to know what they are replying to), **detect their intent** (what they want from you), then **only reply** if that person **exists in HubSpot** matched by **Sales Navigator lead URL**. When you do reply, the **content must follow the detected intent** and the **conversation framework** in `workspace/skills/messaging-guidelines/SKILL.md`. If they show intent to **book a meeting** or **schedule a call** (and HubSpot match passed), include your **HubSpot Meetings** booking link (canonical URL below — send it **only** in-thread after the gate, never to unknown leads). If they are **not** in HubSpot (no matching `linkedin_sales_lead_url`), **do not reply** — leave the thread as-is or mark read per user preference without sending.

## Step 1 — Open Sales Navigator Inbox (Unread)

1. Open in the browser (**`filter=UNREAD`** is applied in the URL — do not substitute the plain inbox URL):
...
**Before composing any reply:** read `workspace/skills/messaging-guidelines/SKILL.md` and follow the conversation framework, tone rules, and reply examples in full.
...
2. **Detect intent** — decide what they want **before** you write a reply. Use labels that fit the copy, for example:
   - **Interested / positive** — wants more info or a next step (if not yet asking to book, you may still **offer** the meetings link as a soft CTA when appropriate, but only after the conversation framework has been followed — see messaging-guidelines).
   - **Book meeting / schedule call** — explicit or strong implicit ask (e.g. "let's book," "send your calendar," "when can we talk?"). After HubSpot match, reply with a **short** confirmation and the **HubSpot Meetings** URL from **HubSpot Meetings — booking a call** below.
   ...
   - **Stop / unsubscribe / not interested** — **no pitch**; brief polite acknowledgment only (or silence per your policy); do not argue. When a HubSpot match exists, **always** update the contact (see **Step 3**): set **`hs_lead_status`** to **`NOT_INTERESTED`**.
   - **Unclear** — one **short** clarifying question is OK if HubSpot match exists and tone stays helpful, not pushy.

   Your **reply must match this intent** (e.g. answer the actual question; if they objected, address that point; do not send a generic template that ignores what they wrote).
...
6. **Reply gate:**
   - **Match found in HubSpot** -> **Compose and send** a reply in the Sales Navigator thread that **directly serves the detected intent**, follows the **conversation framework** in `messaging-guidelines`, uses pathway context from HubSpot notes, and continues uncovering operational pain naturally. Draft the **entire** message first, then insert it into the composer **once** (see **Composer workflow** under **Message format**). If intent is **Book meeting / schedule call** (or equivalent), **include** `https://meetings-ap1.hubspot.com/mostly` in the message body as specified in **HubSpot Meetings — booking a call**. After sending, **Step 3** requires **`hs_lead_status`** -> **`MEETING_LINK_SENT`**.
   - **Match found + Stop / unsubscribe / not interested** — You may send **no** LinkedIn message (silence) or only a brief acknowledgment. Either way, **Step 3** still applies: **`PATCH`** **`hs_lead_status`** -> **`NOT_INTERESTED`**.
...
- **Composer workflow:** Write the **full** message **first** (complete text, including where line breaks belong), **then** put it into the compose box **in one step** — paste the entire body at once, or select-all and replace if updating. Do **not** enter the message **line-by-line** or in many small keystrokes.
- **Tone requirement:** Follow `workspace/skills/messaging-guidelines/SKILL.md` — keep replies casual, human, conversational, low pressure. Write like a real person in a DM, not a formal email or rigid template. No marketing language, no mention of AI, no hard pitch.
...
- **Forbidden in the sent message:** the two-character sequences **`\n`**, **`\n\n`**, **`\t`**, **`\r`**, or any other backslash-escape meant for JSON/code strings. Use real Enter/Return line breaks in the composer instead.
...
- **When:** Intent is **Book meeting / schedule call** (or they clearly pivot to booking in the same thread after you answered a question). **Never** send this link **before** the HubSpot **`linkedin_sales_lead_url`** match passes.
- **How:** One or two sentences of context, then the **full URL** on its own line (use a **real** line break in the composer) so they can tap/open it. No fake or placeholder links.
- **If unsure:** They asked a mix of questions **and** timing — answer the questions briefly, **then** offer the link as the natural next step. Follow the soft call/audit transition patterns in `messaging-guidelines`.
