# Lead follow-up (HubSpot `OPEN` → Sales Navigator)
...
**Body (no subject — this is a reply in the existing thread)**
...
**Send:** In the open Sales Navigator message thread panel, type the body into the composer, click **Send**, and confirm the message appears in the thread.
...
**`outreach_conversation`:** **Append** (do not replace). Preserve existing text; add a separator block, e.g. `---`, label **`LinkedIn follow-up`**, same ISO time, and the full body (no subject).
